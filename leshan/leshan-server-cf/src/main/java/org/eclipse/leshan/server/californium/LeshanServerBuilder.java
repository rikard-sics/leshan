...
public class LeshanServerBuilder {
	 ...
	 * {@link SecurityInfo}.
	 * </p>
	 * By default no security store is set. It is needed for secured connection
	 * if you are using the defaultAuthorizer or if you want PSK feature
	 * activated. An {@link InMemorySecurityStore} is provided to start using
	 * secured connection.
	 * 
	 */
	...
	/**
	 * <p>
	 * Set your {@link Authorizer} implementation to define if a device if
	 * authorize to register to this server.
	 * </p>
	 * By default the {@link DefaultAuthorizer} implementation is used, it needs
	 * a security store to accept secured connection.
	 */
	...
	/**
	 * <p>
	 * Set the {@link PublicKey} of the server which will be used for
	 * RawPublicKey DTLS authentication.
	 * </p>
	 * This should be used for RPK support only. If you support RPK and X509,
	 * {@link LeshanServerBuilder#setCertificateChain(X509Certificate[])} should
	 * be used.
	 */
	 ...
	 * authentication.
	 * </p>
	 * For RPK the public key will be extract from the first X509 certificate of
	 * the certificate chain. If you only need RPK support, use
	...
	/**
	 * <p>
	 * Set the {@link LwM2mNodeDecoder} which will decode data in supported
	 * content format to create {@link LwM2mNode}.
	 * </p>
	...
	/**
	 * deactivate PresenceService which tracks presence of devices using LWM2M
	 * Queue Mode. When Queue Mode is deactivated request is always sent
	 * immediately and {@link ClientSleepingException} will never be raised.
	 * Deactivate QueueMode can make sense if you want to handle it on your own
	 * or if you don't plan to support devices with queue mode.
	 */
	...
	/**
	 * Sets a new {@link ClientAwakeTimeProvider} object different from the
	 * default one.
	 * <p>
	 * By default a {@link StaticClientAwakeTimeProvider} will be used
	 * initialized with the <code>MAX_TRANSMIT_WAIT</code> value available in
	 * CoAP {@link NetworkConfig} which should be by default 93s as defined in
	...
	/**
	 * Sets a new {@link RegistrationIdProvider} object different from the
	 * default one (Random string).
	 * 
	 ...
	 * <p>
	 * There is some use cases where device can have a dynamic IP (E.g. NAT
	 * environment), the specification says to use an UPDATE request to notify
	 * server about IP address/ port changes. But it seems there is some rare
	 * use case where this update REQUEST can not be done.
	 * <p>
	 * With this option you can allow Leshan to update Registration on observe
	 * notification. This is clearly OUT OF SPECIFICATION and so this is not
	 * recommended and should be used only if there is no other way.
	 * 
	 * For {@code coap://} you probably need to use a the Relaxed response
	 * matching mode.
	 * 
	 * <pre>
	...
	public LeshanServer build() {
		...
		if (!noSecuredEndpoint && shouldTryToCreateSecureEndpoint()) {
			if (dtlsConfigBuilder == null) {
				dtlsConfigBuilder = DtlsConnectorConfig.builder(dtlsConfig);
			}
		...
		// we try to build the dtlsConfig, if it fail we will just not
		// create the secured endpoint
		try {
	...
	/**
	 * @return true if we should try to create a secure endpoint on
	 *         {@link #build()}
	 */
	 ...
	 * <p>
	 * You can extend <code>LeshanServerBuilder</code> and override this method
	 * to create a new builder which will be able to build an extended
	 * <code>LeshanServer</code>.
	 * 
	 * @param unsecuredEndpoint CoAP endpoint used for <code>coap://</code>
	 *            communication.
	 * @param securedEndpoint CoAP endpoint used for <code>coaps://</code>
	 *            communication.
	 * @param registrationStore the {@link Registration} store.
	 * @param securityStore the {@link SecurityInfo} store.
	 * @param authorizer define which devices is allow to register on this
	 *            server.
	 ...
	 * @param awakeTimeProvider to set the client awake time if queue mode is
	 *            used.
	 * @param registrationIdProvider to provide registrationId using for
	 *            location-path option values on response of Register operation.
	 * 
	 * @return the LWM2M server
	 */
